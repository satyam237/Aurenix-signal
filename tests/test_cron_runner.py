...
from scheduler.cron_runner import should_run_now
...
def test_should_run_now_matches_hour() -> None:
    ...
    with patch("scheduler.cron_runner.get_settings", return_value=mock_settings):
        assert should_run_now(force=False) is True


def test_cron_runner_main_force() -> None:
    with patch("scheduler.cron_runner.run_pipeline") as mock_run:
